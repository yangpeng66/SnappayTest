resource "aws_security_group" "create_elasticsearch" {
  name = "create_elasticsearch"
...
resource "aws_instance" "elasticsearch-cluster" {
  ...
  provisioner "remote-exec" {
    script        = "${path}/setup-elasticsearch.sh"
  ...
  depends_on = ["aws_security_group.create_elasticsearch"]
}
